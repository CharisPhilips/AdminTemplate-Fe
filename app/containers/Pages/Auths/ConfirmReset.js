...
import { ConfirmResetForm, Notification } from 'kilcote-components';
...
class ConfirmReset extends React.Component {
  ...
  submitForm(values) {
    try {
      Webapis.confirmReset(values.get('confirmationToken'), values.get('password')).then((response) => {
        const { intl } = this.props;
        this.showNotification(intl.formatMessage({ id: 'app.notif.reset.success', defaultMessage: "Your password is changed" }), 'info');
      },
        (error) => {
  ...
  render() {
    const title = constants.BRAND_NAME + ' - ConfirmReset';
      ...
      <div className={classes.root}>
        ...
        <div className={classes.container}>
          <div className={classes.userFormWrap}>
            <ConfirmResetForm onSubmit={(values) => this.submitForm(values)} />
          </div>
        </div>
      </div>
    );
...
export default withStyles(styles)(injectIntl(ConfirmReset));
